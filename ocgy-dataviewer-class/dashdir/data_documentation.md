...
`grep -v '^//' IDP2021_v2_GEOTRACES_IDP2021_Seawater_Discrete_Sample_Data_v2_Y0ihDm4W.txt  >  header_removed_data_.txt`    
- Use the following command to remove all the columns you do not neeed:
```console
awk -F'\t' 'NR==1 {
    for(i=1;i<=NF;i++) {
        h=$i;
        if (h !~ /^QV/ && h !~ /FISH/ && h !~ /UWAY/ && h !~ /DEV/ && h !~ /PUMP/ && h !~ /Type/ && h !~ /Bot/ && h !~ /Name/ && h !~ /Period/ && h !~ /Scientist/ && h !~ /Aliases/ && h !~ /Link/ && h !~ /BODC/) keep[i]=1;
    }
    first=1;
    for(i=1;i<=NF;i++) {
        if (keep[i]) {
...
NR>1 {
    ...
    print "";
}' header_removed_data_.txt > filtered_data_.txt
```
- The new .txt is converted to a csv using Excel (after removing salinity column if necessary and combining Nitrate columns)
